...
# origin_predict_df 는 중복을 제거한 df와 동일한 것을 사용
origin_predict_df = predict_df.copy()
predict_df_ids = predict_df["id"].values

# origin_df에서 predict_df의 id를 제외한 나머지를 train, test로 분리
origin_train_df = origin_df[~origin_df["id"].isin(predict_df_ids)]
origin_train_df, origin_test_df = train_test_split(origin_df, test_size=1 / 9, shuffle=True, stratify=target, random_state=42)
...
print("origin 데이터셋", len(origin_train_df), len(origin_test_df), len(origin_predict_df))
print("중복 제거 데이터셋", len(train_df), len(test_df), len(predict_df))
